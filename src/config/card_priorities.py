...
# 默认高优先级卡牌（在可用费用内优先使用）（数字小优先），再按费用从高到低
...
def get_current_available_cost(device_state, detect_extra_cost_point, pc_controller, image):
    """
    桥接函数：通过 card_priorities 模块访问 utils/game_cost.py 的可用费用计算
    """
    from src.utils.game_cost import get_available_cost as _get_available_cost
    return _get_available_cost(device_state, detect_extra_cost_point, pc_controller, image)